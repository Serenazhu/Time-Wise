...
db = sqlite3.connect(
    r"C:\Users\seren\OneDrive\Documents\Python project\Productivity\Time Management app\tm_db.db")
...
# logo & Setup
favicon = Image.open(
    r'C:\Users\seren\OneDrive\Documents\Python project\Productivity\Time Management app\timewise_logo.png')
st.set_page_config(
    page_title="TimeWise",
    page_icon=favicon,
...
def personalized_header():
    ...
    st.markdown(
        f'<p style="{style}">Welcome to TimeWise {st.session_state.u_login}!</p>', unsafe_allow_html=True)
        ...
        if st.button('Sign Up'):
            ...
            if fu.check_empty(un, pw):
                ...
                if un in user_list:
                    ...
                    db.commit()
                    st.success(
                        'User is added successfully! Please log in.', icon="✅")
...
if st.session_state.u_login == None:
    ...
    personalized_header()
# streamlit run "Time Management app/tm.py"
# Header
...
with col1:
    selected_task = st.selectbox(
        'Choose Your Task:', fu.task_list(un), help="Select from list")
    ...
    if selected_task == "Add Custom Task":
        if st.session_state.u_login == None:
            ...
            if custom_task:
                ...
                db.commit()
                st.write(
                    'Success! Please select Add Custom Task again to see added Task ')
        ...
        if st.button('Record'):
            if st.session_state.u_login == None:
                ...
                if hour_interval == 0:
                    ...
                    cur.execute(insert_date_sql, (un, selected_task, selected_date_string,
                                selected_start_time, selected_end_time, duration2, t, seconds))
                    db.commit()
...
with col3:
    ...
    with st.form('OR You Can Use a Stopwatch'):
        ...
        if st.form_submit_button('Start'):
            ...
            if selected_task != "Add Custom Task":
                st.write('Stopwatch Started! Go ' +
                         selected_task.lower() + ":smile:")
            else:
        ...
        if st.form_submit_button('Stop & Record'):
            if st.session_state.u_login == None:
                ...
                if st.session_state.duration == 0:
                    ...
                    insert_date_sql = "INSERT INTO journal (username, task, date, start, end, duration, type, seconds) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                    cur.execute(insert_date_sql, (un, selected_task, selected_date_string,
                                selected_start_time, selected_end_time, duration, t, seconds))
                    db.commit()
...
with col4:
    image = Image.open(
        r'C:\Users\seren\OneDrive\Documents\Python project\Productivity\Time Management app\Stopwatch.png')
...
st.write(
    '<style>div.stButton > button:first-child {background: linear-gradient(to right,#c5e6fc, #8ad0ff);}</style>', unsafe_allow_html=True)
